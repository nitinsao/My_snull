...
int snull_open(struct net_device *dev)		// is called whenever ifconfig activates it.
{
	 ...
	 */
	printk(KERN_ALERT "%s called, ETH_ALEN = %d", __FUNCTION__, ETH_ALEN);
...
{
	printk(KERN_ALERT "%s called with irq: %i", __FUNCTION__, irq);
	int statusword;
...
static void snull_hw_tx(char *buf, int len, struct net_device *dev)
{
	...
	printk(KERN_ALERT "%s called, buf : %s, len : %i", __FUNCTION__, buf, len);
...
/* Full packet (protocol headers and all) is contained in a socket buffer ( sk_buff ) structure*/
{
	...
	printk(KERN_ALERT "%s called, data : %s, len : %i < ETH_ZLEN: %i", __FUNCTION__, data, len, ETH_ZLEN);
